...
# Проверка и авторизация пользователя при запуске
def check_user_auth():
    ...
    # Запрос никнейма и пароля
    nickname = input("Введите никнейм: ")
    password = input("Введите пароль: ")

    # Хеширование введенных данных
    combined = nickname + password
    hash_input = hashlib.sha256(combined.encode('utf-8')).hexdigest()

    # Сравнение с содержимым файла
    if hash_input == content:
        print(Fore.GREEN + "Авторизация прошла успешно." + Style.RESET_ALL)
    else:
        print(Fore.RED + "Неверный никнейм или пароль. Завершение работы." + Style.RESET_ALL)
        sys.exit()

# Вызов проверки авторизации перед запуском основной программы
check_user_auth()

# Далее идёт остальной код программы из MEMORY-1.2(R).txt
def cl():
...
async def help_cmd2():
    ...
    print("help2 - показать эту страницу")
