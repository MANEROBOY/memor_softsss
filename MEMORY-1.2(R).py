...
def confirm_exit():
    root = tk.Tk()
    root.withdraw()
    result = messagebox.askquestion("начать процесс?", "начать процесс выполнения команды disk -real?", icon='question')
    root.destroy()
    return result == 'yes'
        ...
        if command.lower() == 'exit':
            ...
            try:
                ...
                if 1 <= choice <= len(notes):
                    ...
                    del_choice = input("Удалить заметку? (yes/no): ").lower()
                    if del_choice == 'yes':
                        os.remove(filename)
            ...
            filename = command.split(' ', 1)[1]
            action = input(Fore.MAGENTA + "Очистить всё? (yes/no): " + Style.RESET_ALL).lower()
            try:
                if action == 'yes':
